...
(function () {
  ...
  function popupHtml(item) {
    ...
    const intelBtn = id
      ? '<button type="button" onclick="openPanel(\'' + id + '\')" style="margin-top:4px;margin-right:8px;padding:4px 10px;border-radius:6px;border:0;background:#22c55e;color:#0b1220;font-weight:700;cursor:pointer">Intel</button>'
      : '';
  ...
  function injectLayerUI(sites) {
    ...
    function groupHtml(id, title, subs, defaultOpen) {
      const rows = Object.keys(subs).sort().map(function (sub) {
        ...
        const emoji = fieldEmoji[sub] || '📍';
        return (
          '<label class="layer-item field-sub">' +
    ...
    wrap.innerHTML =
      '<div class="layers-divider" style="height:1px;background:#1e3a55;margin:6px 0"></div>' +
      groupHtml('opsGroupField', 'Field', fieldSubs, true) +
      groupHtml('opsGroupEnt', 'Entertainment', entSubs, true) +
      '<div class="layers-hint" style="padding:8px 14px;font-size:.68rem;color:#6b7f9a">Field & Entertainment on by default.</div>';
    ...
    return fetch('./data/field-sites.json')
      ...
      .then(function (data) {
        ...
        list.forEach(function (s) {
          ...
          const marker = L.marker(ll, { icon: makeFieldIcon(sub) });
          marker.bindPopup(popupHtml(s));
          marker.on('click', function () {
            if (typeof window.openPanel === 'function') window.openPanel(s.id);
          });
          lg.addLayer(marker);
        });
        Object.values(fieldLayerGroups).forEach(function (lg) {
          if (!m.hasLayer(lg)) lg.addTo(m);
        });
        injectLayerUI(list);
      })
      .catch(function (err) {
        console.warn('Field layers:', err);
      });
  }
